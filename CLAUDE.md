...
> 本文件為 kintone 供應商編號（流水號）系統之完整設計與實作說明，適供後續開發、維護及人員交接參考。
> 核心設計原則：**將「發號」與「業務記錄」徹底解耦**，發號作業由一個獨立的計數器 App 統一管理，並藉由樂觀鎖（revision）機制確保號碼唯一且不重複。
...
**正確做法**：編號應來自一個只增不減的獨立計數器（概念類似資料庫 `AUTO_INCREMENT` 或統一發票發號機），與實際業務記錄筆數無關。查詢複雜度恆為 O(1)（透過複合鍵定位單筆記錄），不受業務資料量影響。
...
- 不同種類的計數器對應不同筆記錄，各自獨立運作、互不競爭；revision 樂觀鎖僅在「同一種編號被同時發號」的情境下介入仲裁。
- 導入「適用 App ID」欄位後，定位鍵由單鍵（key）升級為**複合鍵（app_id + key）**，使不同 App 即使使用相同的類別代碼亦不會衝突，使此 Counter App 得以成為**全公司共用的統一發號中台**。

### 3.2 欄位設計

> 欄位顯示名稱（中文，供人員閱讀）與欄位代碼（英文，供程式引用）採分離設計，符合 kintone 最佳實踐原則。
...
> **使用情境**：同一套發號機制，A App 需要 `RN000001` 格式，B App 需要 `RN-001` 或 `2026-RN-001` 格式。
> 編號樣式不應寫死於程式碼中——樣式屬於每一台發號機的個別屬性，因此儲存於 **Counter App 的 `number_format` 欄位**，由後台維護人員直接編輯該筆記錄即可變更樣式，**無須修改 JavaScript 程式，亦無須重新封裝外掛**。
...
| `{自訂名稱}`     | 外掛「代碼對照表」由某欄位值轉出的代碼（逐筆變動的代碼專用，如級別 G/M） | `{level}` → `G`         |

> **`{自訂名稱}` 補充**：`{prefix}` 之類的值固定於計數器記錄上，無法表達「同一台計數器、但每筆記錄不同」的代碼（例如事故級別）。此類需求在外掛設定畫面的「代碼對照表」宣告 `token 名稱 ← 欄位 + 值對照`，發號時由記錄即時解析。若樣板用到該 token 但記錄的欄位值查無對應代碼，發號會中止並提示，不會產出殘缺號碼。
...
### 為何選擇在 `submit.success`（儲存成功後）發號，而非在 `submit` 階段

若於 `submit` 階段即發號，一旦使用者事後取消操作或驗證失敗，號碼已由計數器消耗，將造成跳號現象。待記錄確實寫入後（`submit.success`）再執行發號，可有效避免不必要的號碼損耗。相應的代價是額外增加一次 API 回寫呼叫。
...
- **API Token 不適用於前端環境**。Token 一旦寫入前端 JavaScript，執行時將出現於使用者瀏覽器的網路請求中，透過開發者工具即可取得，存在嚴重資安風險。
- **封裝為 kintone Plugin（外掛）同樣無法保護 Token**。外掛本質上僅是將 JavaScript 打包並提供設定介面，執行時其程式碼仍在**前端瀏覽器**運行，Token 於發出 API 請求時仍會曝露。外掛所解決的是「設定彈性與部署便利性」，**並非資訊安全問題**。

> 結論：API Token 的安全保管，唯一可行的方案是由使用者無法存取的後端服務持有。在無後端環境的前提下，此選項不予採用。
...
建議封裝，但**目的在於設定彈性，而非安全性提升**：允許管理者透過設定介面填寫 Counter App ID、各分類前綴、觸發條件等參數，免除每次調整都需修改程式碼，符合組態驅動（CONFIG-driven）的設計原則。若採外掛形式，內部發號呼叫仍應使用 `kintone.api()`（沿用使用者登入權限），不應在外掛設定中存放 Token。
...
1. **發號與業務記錄解耦**：Counter App 為固定筆數的精簡資料表，不隨業務量成長，發號查詢複雜度恆為 O(1)。
2. **複合鍵（app_id + category_key）定位**：各 App 命名空間相互隔離，同一份程式碼部署至任何 App 皆無需修改，可作為全公司通用發號中台。
3. **revision 樂觀鎖確保唯一性**：同種編號同時觸發發號時，由 kintone 後端序列化仲裁，衝突方自動重試並取得下一個號碼。
4. **國內供應商走旁路**：統一編號為廠商稅籍識別碼，屬「驗證後抄錄」性質，非系統流水發號，不經過 Counter App。
5. **支援週期歸零**：依 `reset_cycle` 設定，跨年、跨月或跨日時自動將流水號歸零重算；歸零動作包含於樂觀鎖範圍內，確保操作原子性；無歸零需求者設 `NONE` 即可永久累加。
6. **純前端環境不採用 Token**：安全性依賴 kintone 原生權限控制與編號欄位唯一性約束；樂觀鎖機制仍完整保障號碼不重複發出。
